...
//        solution = new Solution1();
    ...
    public static Collection<Object[]> testcases() {
        return Arrays.asList(new Object[][]{
                {"[1,2,3,null,5,null,4]", Arrays.asList(1, 3, 4)},
                {"[1,null,3]", Arrays.asList(1, 3)},
                {"[]", Arrays.asList()},
                {"[1,2,3,null,5]", Arrays.asList(1, 3, 5)}
        });
    }
    ...
    public void test() {
        ...
        Object actual = solution.rightSideView(TreeNodeConvertClass.stringToTreeNode(para1));

        assertThat(actual, is(equalTo(expected)));

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("rightSideView() pass unit test!");
        }
    }
