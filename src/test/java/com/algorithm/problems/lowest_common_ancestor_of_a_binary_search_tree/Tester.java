...
public class Tester {
    ...
    public void test() {
        ...
        Object actual = solution.lowestCommonAncestor(
                TreeNodeConvertClass.stringToTreeNode(para1),
                TreeNodeConvertClass.stringToTreeNode(para2),
                TreeNodeConvertClass.stringToTreeNode(para3)
        );

        assertThat(((TreeNode) actual).val, is(equalTo(expected)));

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("lowestCommonAncestor() pass unit test!");
        }
    }
